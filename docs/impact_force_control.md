...
动力学方程求出的 `qdd` 会继续积分成新的关节速度和关节位置：
...
这个新的 `q/qd` 不是输出曲线里的附属结果，而是下一次仿真循环的“真实机械臂状态”。后续所有控制量都会重新从它出发计算：

- 用 `q` 做正运动学，得到末端实际位置 `x`；
- 用 `q/qd` 和雅可比矩阵，得到末端实际速度 `xd`；
- 用当前 `q` 重新计算 `J`、`M(q)`、`C(q,qd)` 和 `G(q)`；
- 用末端实际位置判断是否压入目标、是否产生接触力；
- 用实际状态和期望状态的误差重新计算 `tau_cmd`。

因此 `q_des` 表示“计划希望机械臂去哪”，而动力学积分后的 `q` 表示“在控制力矩、惯性、重力、阻尼和接触反作用共同作用下，机械臂实际上到了哪”。接触力如果把末端顶住，下一步的 `q` 就会落后于参考轨迹；控制器会基于这个真实误差重新给出力矩。这就是把接触力放进动力学方程的核心意义。
...
状态机决定当前应该生成哪一类参考命令，但每个仿真步内部还会形成一条更细的闭环：

```text
当前实际关节状态 q, qd
  -> 正运动学得到末端位置 x
  -> 雅可比得到末端速度 xd
  -> 根据 phase 生成 x_des, xd_des
  -> 操作空间阻抗控制生成 tau_cmd
  -> 接触模型根据 x, xd 生成 F_contact
  -> J'F_contact 变成关节外力矩 tauContact
  -> 关节动力学求 qdd
  -> 积分得到新的 q, qd
  -> 下一步继续反馈
```

也就是说，规划轨迹提供的是参考目标，动力学积分得到的是实际状态，接触模型提供的是环境反作用，控制器每一步都在实际状态和参考目标之间重新闭环。
