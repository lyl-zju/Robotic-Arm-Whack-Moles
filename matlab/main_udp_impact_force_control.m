...
requestedUdpLocalPort = [];
if exist("udpLocalPort", "var")
    requestedUdpLocalPort = udpLocalPort;
end

requestedDemoMaxRuntime = inf;
if exist("udpDemoMaxRuntime", "var")
    requestedDemoMaxRuntime = udpDemoMaxRuntime;
elseif strlength(string(getenv("UDP_DEMO_MAX_RUNTIME"))) > 0
    requestedDemoMaxRuntime = str2double(getenv("UDP_DEMO_MAX_RUNTIME"));
end

clearvars -except requestedHitMotionMode requestedUdpLocalPort requestedDemoMaxRuntime; clc; close all;
...
if isempty(requestedUdpLocalPort)
    udpLocalPort = 5005;
else
    udpLocalPort = requestedUdpLocalPort;
end
udpPollPeriod = 0.005;
idlePrintPeriod = 2.0;
receiver = udpport("datagram", "IPV4", ...
    "LocalPort", udpLocalPort, "Timeout", udpPollPeriod);
cleanupReceiver = onCleanup(@()cleanup_udp_receiver(receiver));
...
[boardFig, boardAx, targetMarker, eeMarker, boardHud] = create_board_window(cfgBoard);
...
show_robot_frame(robotAx, robot, qAct, cfgBoard, currentTargetWorld, ...
    ...
    force_visual_state(forceState, tCurrent, currentTargetId, currentTargetSource), forceHud);
update_board_hud(boardHud, currentTargetId, currentTargetSeq, currentTargetSource, ...
    "waiting", forceState);

fprintf("UDP impact force-control simulation started.\n");
fprintf("Motion mode: %s\n", cfgController.motion_mode);
fprintf("Listening for Python UDP board targets on 0.0.0.0:%d\n", udpLocalPort);
fprintf("Start Python after this script is running. Targets received during motion are held as the latest pending target.\n");
fprintf("Press Esc in the board window, or close a figure, to stop.\n");
if isfinite(requestedDemoMaxRuntime)
    fprintf("Auto-stop enabled after %.2f s.\n", requestedDemoMaxRuntime);
end

while is_simulation_running(boardFig, robotFig) && tCurrent < requestedDemoMaxRuntime
    stepTimer = tic;
    drawnow limitrate;

    incomingUdpTarget = read_latest_udp_board_target(receiver, cfgBoard);
    udpTarget = [];
    if ~isempty(incomingUdpTarget)
        lastIdlePrint = tic;
        if isempty(activeTraj)
            udpTarget = incomingUdpTarget;
        else
            pendingUdpTarget = incomingUdpTarget;
        end
    elseif isempty(activeTraj) && ~isempty(pendingUdpTarget)
        udpTarget = pendingUdpTarget;
        pendingUdpTarget = [];
    end

    if ~isempty(udpTarget)
        targetBoard = udpTarget.position_board;
        targetWorld = udpTarget.position_world;
        qStart = qAct;
        qdStart = qdAct;
        ...
        try
            ...
            currentTargetId = udpTarget.id;
            currentTargetSeq = udpTarget.seq;
            currentTargetSource = char(string(udpTarget.source));
            ...
            set(targetMarker, "XData", targetBoard(1), "YData", targetBoard(2));
            update_board_hud(boardHud, currentTargetId, currentTargetSeq, currentTargetSource, ...
                "tracking", forceState);
            title(boardAx, sprintf("UDP target id=%d, board=[%.3f %.3f], source=%s", ...
                currentTargetId, targetBoard(1), targetBoard(2), currentTargetSource), ...
                "Color", [0.0 0.45 0.15]);

            fprintf("t=%.2f s: UDP target seq=%g id=%d board=[%.3f %.3f], waypoints=%d\n", ...
                tCurrent, currentTargetSeq, currentTargetId, ...
                currentTargetBoard(1), currentTargetBoard(2), size(activeTraj.q_waypoints, 1));
        catch err
            warning("main_udp_impact_force_control:ReplanFailed", ...
    ...
    elseif isempty(activeTraj) && isempty(pendingUdpTarget) && toc(lastIdlePrint) >= idlePrintPeriod
        fprintf("Waiting for UDP target packets...\n");
        lastIdlePrint = tic;
    end
    ...
    if forceState.success && ~isempty(currentTargetId) && ...
            (isempty(lastSuccessSeq) || ~isequaln(currentTargetSeq, lastSuccessSeq))
        lastSuccessSeq = currentTargetSeq;
        update_board_hud(boardHud, currentTargetId, currentTargetSeq, currentTargetSource, ...
            "hit_success", forceState);
        title(boardAx, sprintf("hit success: target id=%d, peak force=%.2f N", ...
            currentTargetId, forceState.peak_force), "Color", [0.02 0.45 0.12]);
        fprintf("t=%.2f s: hit success target id=%d, peak force=%.2f N, source=%s\n", ...
            tCurrent, currentTargetId, forceState.peak_force, currentTargetSource);
    end
...
fprintf("UDP impact force-control simulation stopped at t=%.2f s.\n", tCurrent);

function cleanup_udp_receiver(receiver)
try
    clear receiver;
catch
end
...
function [fig, ax, targetMarker, eeMarker, hud] = create_board_window(cfgBoard)
fig = figure("Name", "UDP Impact Force-Control Target Board", ...
    "Color", "w", "NumberTitle", "off");
ax = axes("Parent", fig);
axes(ax);

draw_board(cfgBoard, []);
title(ax, "waiting for Python UDP target packets", ...
...
hud.text = annotation(fig, "textbox", [0.02 0.72 0.36 0.22], ...
    "String", "target id: waiting", ...
    "FitBoxToText", "off", ...
    "FontName", "Consolas", ...
    "FontSize", 10, ...
    "FontWeight", "bold", ...
...
function update_board_hud(hud, targetId, targetSeq, targetSource, status, forceState)
if isempty(hud) || ~isfield(hud, "text") || ~isgraphics(hud.text)
    return;
end

if isempty(targetId)
    hudText = sprintf("target id: waiting\nstatus   : %s", char(string(status)));
    color = [0.04 0.05 0.06];
else
    seqText = "n/a";
    if ~isempty(targetSeq) && isfinite(targetSeq)
        seqText = sprintf("%.0f", targetSeq);
    end
    peakForce = 0.0;
    if ~isempty(forceState) && isfield(forceState, "peak_force")
        peakForce = forceState.peak_force;
    end
    if status == "hit_success"
        color = [0.02 0.42 0.12];
    else
        color = [0.04 0.05 0.06];
    end
    hudText = sprintf([ ...
        ...
        'peak F   : %.2f N\n', ...
        'source   : %s'], ...
        targetId, char(string(seqText)), char(string(status)), ...
        peakForce, char(string(targetSource)));
end

set(hud.text, "String", hudText, "Color", color);
end

function [fig, ax, hud] = create_robot_window()
fig = figure("Name", "UDP Impact Force-Control Robot", ...
    "Color", "w", "NumberTitle", "off");
ax = axes("Parent", fig);
...
function visual = force_visual_state(forceState, tCurrent, targetId, targetSource)
if nargin < 3
    targetId = [];
end
if nargin < 4
...
visual.t = tCurrent;
visual.target_id = targetId;
visual.target_source = char(string(targetSource));
...
targetTitle = "target id = waiting";
if isfield(forceVisual, "target_id") && ~isempty(forceVisual.target_id)
    targetTitle = sprintf("target id = %d", forceVisual.target_id);
end
if forceVisual.success
    targetTitle = sprintf("%s, hit success", targetTitle);
end
title(ax, sprintf("UDP impact force control, %s, t = %.2f s, F_n = %.2f N, peak = %.2f N", ...
    targetTitle, tCurrent, forceVisual.force, forceVisual.peak_force));
drawnow limitrate;
catch err
    if isgraphics(ax)
        rethrow(err);
...
figure(fig);
hud.text = annotation(fig, "textbox", [0.02 0.58 0.36 0.40], ...
    "String", "", ...
    "FitBoxToText", "off", ...
    "FontName", "Consolas", ...
    "FontSize", 9, ...
    ...
    "LineWidth", 0.8, ...
    "Margin", 6);
hud.barBack = annotation(fig, "rectangle", [0.04 0.545 0.30 0.024], ...
    "FaceColor", [0.86 0.86 0.86], ...
    "Color", [0.30 0.30 0.30], ...
    "LineWidth", 0.7);
hud.barFill = annotation(fig, "rectangle", [0.04 0.545 0.001 0.024], ...
...
if isempty(hud) || isempty(state) || ~state.has_force
    set(hud.text, "String", sprintf("target id      waiting\nstatus         waiting for Python UDP target"));
    set(hud.barFill, "Position", [0.04 0.545 0.001 0.024], ...
        "FaceColor", [0.85 0.18 0.12]);
    return;
end
...
targetText = "waiting";
if isfield(state, "target_id") && ~isempty(state.target_id)
    targetText = sprintf("%d", state.target_id);
end
successText = "running";
if state.success
    successText = "HIT SUCCESS";
end
sourceText = "";
if isfield(state, "target_source")
    sourceText = char(string(state.target_source));
end
hudText = sprintf([ ...
    'target id      %s\n', ...
    'hit status     %s\n', ...
    ...
    'phase          %s\n', ...
    'source         %s'], ...
    char(string(targetText)), char(string(successText)), state.force, state.filtered_force, ...
    state.peak_force, state.threshold, state.target_force, phaseText, sourceText);

set(hud.text, "String", hudText);
set(hud.barFill, "Position", [0.04 0.545 0.30 * fillRatio 0.024], ...
    "FaceColor", barColor);
end
